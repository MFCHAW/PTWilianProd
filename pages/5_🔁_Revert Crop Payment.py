...
if 'revert_status' not in st.session_state:
    st.session_state['revert_status'] = ''
    
if 'revert_message' not in st.session_state:
    st.session_state['revert_message'] = ''
...
def revertPayment(ou, batch):
    ...
    st.session_state['revert_status'] = 'Process'
    st.session_state['revert_message'] = 'Revert Crop payment posting in progress...'
...
def show_MainPage():
    ...
    # with mainSection:
        if st.session_state['revert_status'] == '':
            ou = st.selectbox(
                'Mill: ',
            ...
            st.button('Process',
                    on_click=revertPayment,
                    args=(ou, batch),
                    help='Click to start to revert the crop payment.')

            st.markdown('#')

            st.write('Please fill in all the information and click "Revert" button.')
...
def show_StatusMsg():
    with statusMsgSection:
        ...
        if st.session_state['revert_status'] == 'Process':
            statusMsg.info(st.session_state['revert_message'])
        elif st.session_state['revert_status'] == 'Succeeded':
            statusMsg.success(st.session_state['revert_message'])
        elif st.session_state['revert_status'] == 'Failed':
            statusMsg.error(st.session_state['revert_message'])
        
def show_ErrorMsg():
    with errorMsgSection:
        if st.session_state['revert_error_message'].any().any():
            # Configure grid options using GridOptionsBuilder
            builder = GridOptionsBuilder.from_dataframe(st.session_state['revert_error_message'])
            ...
            st.table(st.session_state['revert_error_message'])
...
def reset_Form():
    st.session_state['revert_status'] = ''
    st.session_state['revert_message'] = ''
    st.session_state['revert_error_message'] = ''


with pageSection:
    st.title('Revert Crop Payment')
    statusMsg = st.empty()
